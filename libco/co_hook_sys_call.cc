...
typedef int (*gethostbyname_r_pfn_t)(const char *__restrict name, struct hostent *__restrict __result_buf,
...
ssize_t read(int fd, void *buf, size_t nbyte) {
    ...
    if (readret < 0) {
        co_log_err("CO_ERR: read fd %d ret %ld errno %d poll ret %d timeout %d", fd, readret, errno, pollret, timeout);
    }
...
ssize_t recv(int socket, void *buffer, size_t length, int flags) {
    ...
    if (readret < 0) {
        co_log_err("CO_ERR: read fd %d ret %ld errno %d poll ret %d timeout %d", socket, readret, errno, pollret,
                   timeout);
    }
...
int co_gethostbyname_r(const char *__restrict name, struct hostent *__restrict __result_buf, char *__restrict __buf,
